...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *sebtcjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *sebtcjson.NotifyNewTransactionsCmd:
...
// returning an unmarshaled sebtcjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]sebtcjson.OutPoint, 0, nslen)

...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[sebtcjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[sebtcjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *ltcutil.Tx, details *sebtcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *ltcutil.Tx, details *sebtcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *sebtcjson.TxRawResult)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case sebtcjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case sebtcjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case sebtcjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case sebtcjson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case sebtcjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case sebtcjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case sebtcjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case sebtcjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case sebtcjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case sebtcjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case sebtcjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnAccountBalance
	case sebtcjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case sebtcjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseChainTxNtfnParams(params []json.RawMessage) (*ltcutil.Tx,
	*sebtcjson.BlockDetails, error) {
	...
	// JSON object.
	var block *sebtcjson.BlockDetails
	if len(params) > 1 {
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*sebtcjson.TxRawResult,
	error) {
	...
	var rawTx sebtcjson.TxRawResult
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := sebtcjson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []sebtcjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := sebtcjson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the btcjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) sebtcjson.OutPoint {
	return sebtcjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]sebtcjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := sebtcjson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := sebtcjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := sebtcjson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(addresses []ltcutil.Address) FutureNotifyReceivedResult {
	...
	cmd := sebtcjson.NewNotifyReceivedCmd(addrs)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := sebtcjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := sebtcjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.sendCmd(cmd)
}
...
func (c *Client) LoadTxFilterAsync(reload bool, addresses []ltcutil.Address,
	...
	outPointObjects := make([]sebtcjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = sebtcjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := sebtcjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
